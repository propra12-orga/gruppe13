...
public class FeldGP extends JComponent {
	...
	public synchronized void walkFigur(int xStart, int yStart, int xZiel,
			int yZiel) {
		zwSchritt = 0;
		this.xStart = xStart;
		this.yStart = yStart;
		...
		new Thread(new Runnable() {
			...
			public void run() {
				...
				for (int i = 0; i < Start.getTileWidth(); i++) {
					zwSchritt++;
	...
	public void paint(Graphics g) {
		...
		g.drawImage(feld.getFigur(),
				(int) (feld.getTileWidth() * xStart + zwSchritt
						* (xZiel - xStart)), (int) (feld.getTileHeight()
						* yStart + zwSchritt * (yZiel - yStart)), null);
	}
